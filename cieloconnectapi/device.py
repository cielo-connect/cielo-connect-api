...
from .const import *
from .model import CieloDevice


class CieloDeviceAPI:
    """Per-device facade over a shared CieloClient.

    This avoids using shared mutable client.device_data.
    """

    def __init__(self, client: "CieloClient", device: CieloDevice) -> None:
        self._client = client
        self.device_data = device  # per-device context
    ...
    def mode_caps(self) -> dict:
        ...
        mode_key = self.device_data.hvac_mode or "off"
        if mode_key == "aux":
            mode_key = "heat"
    ...
    async def async_send_api_call(self, action_type, action_value) -> dict:
        ...
        response = await self._client.set_ac_state(
